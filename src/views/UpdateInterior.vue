  ...
  <main>
    ...
    <div class="my-5">
      <div class="mx-auto w-25" style="max-width: 100%;">
        <h2 class="text-center mb-3">Update Interior</h2>
        <form @submit.prevent="updateInterior">
          <!--description-->
          <div class="row">
...
export default {
  ...
  methods: {
    getInterior() {
      fetch(`http://localhost:8080/interiors/${this.$route.params.id}`)
          .then(res => res.json())
          .then(data => {
            this.interior = data;
    ...
    updateInterior() {
      if (!this.isFormInvalid) {
        fetch('http://localhost:8080/interiors/' + this.interior.id, {
          method: 'PATCH',
          headers: {
